...
# =============================================================================
# Node 0 -- SAMheraAPIKey
#   Enter API key once, connect to all nodes via noodle
# =============================================================================

class SAMheraAPIKey:
    """Enter your API key once and connect it to all SAMhera nodes."""
...
class VLMImageTest:
    """
    Debug node: sends the image to the VLM and asks it to describe what it sees.
    Use this to verify the VLM is actually receiving and reading the image.
    """
    ...
    def run(self, image, api=None, api_key="", provider="gemini", model_name="gemini-2.5-pro"):
        if api is not None:
            api_key = api["api_key"]; provider = api["provider"]
        pil_img = _tensor_to_pil(image)
